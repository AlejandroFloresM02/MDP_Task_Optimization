...
actions = [[x for x in rec] for rec in csv.reader(open("data/actions.csv"))][0]
actions = dict(zip(actions, [i for i in range(1, len(actions)+1)]))
...
#trans_mat = csv.reader('SM_trans_mat.csv')
...
for iter in range(iter_limit):
    ...
    for state in range(states_amount):
        ...
        for key in actions:
            #print(u_pairs[state])
            action_val = rewards[int(u_pairs[state][0]-1), actions[key]-1]
            for next_state in range(len(state_names)):
                #print(trans_mats[actions[key]-1])
                try:
                    pair_index = findActionPairIndex(trans_mats[actions[key]-1][:,0:2],u_pairs[state])
                except:
                    continue
                if u_pairs[state][0]-1 == next_state:
                    ...
                    backward = 0 
                #print(trans_mats[actions[key]-1][:,0:2])
                try:
                    next_pair_index = findActionPairIndex(trans_mats[actions[key]-1][:,0:2],np.array([next_state+1,backward]))
                except:
                    continue
                #print("state/bckwrd", u_pairs[state])
                #print("next state/bckwrd", trans_mats[actions[key]-1][next_pair_index][0:2])
                action_val += discount_fact * trans_mats[actions[key]-1][pair_index][next_state+2] * state_values[next_state]
            #print("Last for done")
    ...
    for key in actions:
        ...
        for next_state in range(len(state_names)):
            try:
                ...
                continue
            #print(trans_mats[actions[key]-1][pair_index][next_state+2])
            action_val += discount_fact * trans_mats[actions[key]-1][pair_index][next_state+2] * state_values[next_state]
        action_values[actions[key]-1] = action_val
